...
class Split {
    constructor(name, splitTime, globalTime, perCube) {
        this.name = name;
        this.splitTime = splitTime;
        this.globalTime = globalTime;
        this.perCube = perCube;

        splitStorage.push(this);
    }
}
...
class Attempt {
    ...
    createSummary() {
        ...
        let slicedTime = (new Date(this.date)).toString().slice(0, 21);
    
        let clipboardText = `${this.solved}/${this.attempted} MBLD attempt in ${formatTime(this.splits[this.splits.length-1].globalTime+this.plusTwos*200)+"+".repeat(this.plusTwos)}[${formatTime(this.splits[this.splits.length-2].globalTime)}]\nended at ${slicedTime}\n`;
        for (let split of this.splits) {
            switch (split.name) {
                case "Total Memo":
                    clipboardText += "\n";
                case "Total":
                    clipboardText += `\n${split.name} - ${formatTime(split.globalTime)} (${formatTime(split.perCube)}/cube)`;
                    break;
                default:
                    clipboardText += `\n${split.name} - ${formatTime(split.splitTime)} (${formatTime(split.globalTime)}, ${formatTime(split.perCube)}/cube)`;
                    break;
                case "Exec":
                    clipboardText += `\n${split.name} - ${formatTime(split.splitTime)} (${formatTime(split.globalTime)}, ${formatTime(split.perCube)}/cube)${asterisk}`;
                    break;
            }
        }
    
        clipboardText += notesFormatted;
        clipboardText += `${asteriskExplanation}\n\nGenerated by MBLD Timer`;
...
class PastAttemptRow extends HTMLDivElement {
    constructor(attempt) {
        ...
        this.text = this.attempt.solved + "/" + this.attempt.attempted + " " + formatTime(this.attempt.time) + "+".repeat(this.attempt.plusTwos) + " (" + this.attempt.points() + optionalP + formatDate(this.attempt.date);
        ...
        summaryIcon.onclick = function() {
            displayDialog("Summary", attempt.createSummary());
        };
    ...
    if (event.key === " " && !editing && Object.keys(reviewSystemStorage).length !== 0) {
        ...
        switch (unformatTime(timer.textContent)) {
            case hour:
                ...
                totalRow.querySelector(".global-time").textContent = formatTime(time);
                totalRow.querySelector(".per-cube").textContent = formatTime(Math.round(time/totalCubes));
            ...
            default:
                time = hour;
                splitTime = hour - realMemoTimeStorage;
                execRow.querySelector(".split-time").textContent = formatTime(hour - unformatTime(memoRow.querySelector(".global-time").textContent));
                execRow.querySelector(".global-time").textContent = formatTime(hour);
                execRow.querySelector(".per-cube").textContent = formatTime(Math.round(unformatTime(execRow.querySelector(".split-time").textContent)/totalCubes));
                totalRow.querySelector(".global-time").textContent = formatTime(hour);
                totalRow.querySelector(".per-cube").textContent = formatTime(Math.round(hour/totalCubes));
                timer.textContent = formatTime(time);
                plusTwos = 0;
                splitStorage = splitStorage.slice(0, -3);
                saveSplit(true);
...
function saveSplit(finalSplit) {
    let rowsReverse = rows.slice().reverse();

    for (let row of rowsReverse) {
        if (row.querySelector(".split-time").textContent !== "" && !finalSplit) {
            new Split(row.querySelector(".name-input").value, splitTime, time, Math.round(splitTime / row.querySelector(".cube-number input").value));
            break;
        } else if (finalSplit) {
            new Split("Exec", splitTime, time, Math.round(splitTime / execRow.querySelector(".cube-number").textContent));
            new Split("Total Memo", "N/A", time - splitTime, Math.round((time - splitTime) / execRow.querySelector(".cube-number").textContent));
            new Split("Total", "N/A", time, Math.round((time) / execRow.querySelector(".cube-number").textContent));
            break;
        }
    }
}
...
function stopTimer() {
    ...
    realMemoTimeStorage = unformatTime(memoRow.querySelector(".global-time").textContent);
    clearInterval(interval);
    timerState = "done";
    currentSplit = 0;
